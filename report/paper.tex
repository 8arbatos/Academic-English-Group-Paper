...
\IEEEtitleabstractindextext{
  \begin{abstract}
    ...
    Finally, we discuss open challenges, including storage compression and integration with generative AI. We believe this survey serves as a timely roadmap for researchers navigating the rapid paradigm shift from implicit fields to explicit primitives in 4D vision.
...
Following the original NeRF, foundational extensions were quickly proposed to enhance robustness: \textbf{NeRF++} \cite{nerfpp} introduced inverted sphere parameterization to model unbounded scenes, and \textbf{Mip-NeRF} \cite{mipnerf} employed conical frustums to address aliasing artifacts, which was further refined by \textbf{Mip-NeRF 360} \cite{mipnerf360} for large-scale environments. 
Additionally, \textbf{Ref-NeRF} \cite{refnerf} significantly improved the rendering of glossy surfaces by restructuring the view-dependent parameterization.
...
Prior to 3DGS, methods like \textbf{DVGO} \cite{dvgo} and \textbf{Plenoxels} \cite{plenoxels} attempted to accelerate training using explicit voxel grids, while \textbf{NeuS} \cite{neus} and \textbf{VolSDF} \cite{volsdf} focused on improving geometric fidelity using Signed Distance Functions (SDF).
...
Furthermore, \textbf{NR-NeRF} \cite{nr_nerf} demonstrated robust non-rigid reconstruction from monocular video. To handle high-resolution dynamics efficiently, \textbf{HyperReel} \cite{hyperreel} introduced a memory-efficient 6-DoF tensor representation, significantly improving rendering speed compared to pure MLP-based deformations.
...
%----------------------------------------------------------------------------------------
\subsubsection{Specialized and Robust Dynamic Gaussians} 
Beyond general scenes, significant efforts have been directed towards human-centric reconstruction. 
\textbf{GauHuman} \cite{gauhuman} and \textbf{HUGS} \cite{hugs} integrate SMPL priors into the 3DGS pipeline to achieve real-time rendering of articulated humans (60+ FPS). 
\textbf{GaussianAvatar} \cite{gaussianavatar} further enables drivable avatars from monocular video, while \textbf{GPS-Gaussian} \cite{gps_gaussian} introduces a generalizable pixel-wise splatting approach for robust human synthesis.

Specific to head avatars, \textbf{NerFace} \cite{nerface} pioneered dynamic facial modeling with NeRFs. Recent Gaussian-based works like \textbf{FlashAvatar} \cite{flash_avatar}, \textbf{InstaAvatar} \cite{insta_avatar}, and \textbf{MonoGaussianAvatar} \cite{monogaussianavatar} achieve high-fidelity head reconstruction with extreme efficiency.

To enhance physical plausibility and controllability, \textbf{PhysGaussian} \cite{physgaussian} incorporates unified simulation and rendering, allowing 3D Gaussians to respond to physical forces. 
\textbf{Gaussian-Flow} \cite{gaussian_flow} and \textbf{CoGS} \cite{cogs} explicitly model scene flow and controllability to address the limitations of purely deformation-based approaches. 
Furthermore, \textbf{SC-GS} \cite{sc_gs} utilizes sparse control points for editable dynamics, while \textbf{Spacetime Gaussian} \cite{spacetime_gaussian} employs feature splatting to enhance temporal consistency.

Finally, addressing the sampling artifacts in multi-scale rendering, \textbf{Mip-Splatting} \cite{mip_splatting} introduces 3D smoothing filters to enable alias-free rendering. To improve geometric stability, \textbf{Scaffold-GS} \cite{scaffold_gs} utilizes anchor points to guide the distribution of Gaussians.
In the domain of robotics, 3DGS has been successfully adapted for Simultaneous Localization and Mapping (SLAM). Systems like \textbf{GS-SLAM} \cite{gs_slam} and \textbf{Splat-SLAM} \cite{splat_slam} demonstrate that dynamic Gaussians can serve as a unified representation for both tracking and mapping.
...
Similarly, \textbf{Tensor4D} \cite{tensor4d} employs tensor decomposition to model 4D dynamics efficiently. 
For streaming applications, \textbf{StreamRF} \cite{streamrf} proposes a transformation-aware approach to handle long dynamic video sequences with on-the-fly training.
...
To address aliasing artifacts in grid-based methods, \textbf{Zip-NeRF} \cite{zipnerf} combines the speed of grids with the anti-aliasing properties of multi-sampling, setting a new benchmark for static quality. 
For dynamic robustness, \textbf{RoDynRF} \cite{rodynrf} introduces robust loss functions to handle occlusions and outliers in monocular video, significantly improving stability in uncontrolled environments.
...
While 3DGS excels in rendering speed, it is prone to specific artifacts not typically seen in NeRFs. 
Due to the discrete nature of primitives, ``floaters'' (randomly placed opaque Gaussians) often appear in near-camera regions. 
Furthermore, in areas with sparse observations, optimization can lead to ``needle-like'' elongated Gaussians, which degrade visual quality when viewed from novel angles. 
Recent regularization techniques \cite{scaffold_gs, mip_splatting} attempt to mitigate these issues, but they remain an open problem for unconstrained dynamic scenes.
...
\begin{itemize}
  ...
  \item \textbf{Future Direction:} Future work focuses on vector quantization and codebook-based learning. Recent attempts like \textbf{Compact3D} \cite{c3dgs} and \textbf{EAGLES} \cite{eagles} have successfully reduced the storage of Gaussian fields by over $10\times$ without significant quality loss through quantized attribute encoding. 
  Additionally, distinct ``Level of Detail'' (LOD) strategies could be developed to stream dynamic 3D content efficiently over limited bandwidth networks. Furthermore, optimizing the rasterization pipeline for \textbf{WebGPU and Mobile Hardware} is crucial. Enabling high-fidelity 4D rendering on edge devices (VR headsets, smartphones) will be the key to mass adoption in the Metaverse.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Future Direction:} Dynamic Scene Modeling serves as the ideal ``3D lifting'' mechanism for generative AI. Pioneering works like \textbf{DreamFusion} \cite{dreamfusion} utilized SDS loss for NeRF generation. 
    Recently, \textbf{DreamGaussian} \cite{dreamgaussian} adapted this to explicit splatting for minute-level generation. \textbf{LGM} \cite{lgm} further demonstrates the potential of training large-scale transformers to directly predict 3D Gaussians from single images.

      \textbf{Text-to-4D:} Combining text-to-video models with Dynamic 3DGS to generate 4D assets directly from prompts. 
      Seminal works like \textbf{MAV3D} \cite{mav3d} pioneered this direction using NeRFs. 
      More recently, \textbf{Align Your Gaussians} \cite{align_your_gaussians} and \textbf{Animate124} \cite{animate124} leverage the explicit nature of Gaussians to animate static objects or generate 4D scenes from single images with higher fidelity and speed.
